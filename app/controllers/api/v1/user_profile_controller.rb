  ...
  module V1
    class UserProfileController < ::Api::V1::BaseController
      ...
      get "/" do
        ...
        json ::Api::Resources::UserProfileResource.new(UserProfile.where(:user_id => user.id))
      end

      post "/register" do
        halt(404) unless (user = User.find_by(name: @payload["name"]))
        screen_name, description= parse_json_or_halt(request.body.read).values_at(:screen_name, :description)
